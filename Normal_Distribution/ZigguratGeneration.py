...
parser.add_argument('--type', default='normal', choices={'exponential', 'normal'}, help="PRNG type.")
parser.add_argument('--size', default=253, type=int, help="Number of Ziggurat layers.")
...
def fsolve(f, a, b, xtol=1e-19, iterations=500, start_safe=False):
...
"""
    ...
    while i >= 0:
        ...
        if j > L - 1:  # Nobody needs probability mass, Done
            break
        X[B[j]] -= 1 - X[
            B[i]]  # Send all of x_(b_i) excess probability mass to x_(b_j)                (x_(b_i) is now done).
        A[B[i]] = B[j]
...
if args.type == 'exponential':
    ...
    volume = 0.5 / np.longdouble(args.size)
    f = lambda x: (1 / sqrt(2 * pi)) * exp(-oneHalf * x * x)
    CDF = np.vectorize(lambda x: alpha * erf(sqrt(oneHalf) * x))


def generate_X_i(vol=volume, last_Y_i=0):
    lower_bound = 1
    upper_bound = 4 if args.type == 'exponential' else 10
    # X_i = fsolve(lambda x: (2*x+2)/2 * (f(x) - last_Y_i) - vol, lower_bound, upper_bound)
    # yield X_i
    # last_Y_i = f(X_i)
    # lower_bound = 0.5 * X_i
    # upper_bound = X_i
    while lower_bound > vol*0.01:
    # while lower_bound > vol:  # Obtaining another layer is clearly hopeless after this point
        ...
        try:
            X_i = fsolve(lambda x: upper_bound * (f(x) - last_Y_i) - vol, lower_bound, upper_bound)
            yield X_i
            last_Y_i = f(X_i)
            lower_bound = 0.5 * X_i
            upper_bound = X_i
...
Y = np.vectorize(f)(X)
counter=0
while(counter<len(X)//5):

    temp=''
    for i in range(5):
        temp=temp+str(X[i+counter*5])+', '
    print(temp)
    counter += 1
print(X)
counter=0
while(counter<len(Y)//5):

    temp=''
    for i in range(5):
        temp=temp+str(Y[i+counter*5])+', '
    print(temp)
    counter += 1
print(Y)
...
plot(x, f(x))
# plt.fill_between(x,f(x),alpha=0.37)
# scatter(X, Y)
...
for i in range(1,len(X)):
    # i = i + 1
    plot([0, X[i-1]], [Y[i], Y[i]], 'k-', lw=1)
    plot([X[i-1], X[i-1]], [Y[i], Y[i - 1]], 'k-', lw=1)
ax=plt.gca()
ax.set_facecolor('#EBEBEB')
ax.grid(which='major', color='white', linewidth=1)
plt.xlim(0, 3.2)
plt.ylim(0, 0.42)
# plt.title('Ziggurat Method (256 Blocks)')
# plt.savefig('C:/Users/13862/Desktop/ZigguratMethod8Blocks.png')
plt.show()

print(X[0]*Y[0])
print(X[-2]*(Y[-1]-Y[-2])/volume)
print("Chosen area size was 1/{}".format(volume))
print("Lmax is {0}, boxes cover Lmax*area={1} of the region under the curve",len(X))
# for i in range(1, len(X)):
...
#     E = epsilon / dY
#     print(f'Maximum deflection of f(x) relative to tangent line is {E.max()} and occurs above layer {E.argmax()}')
#     iE_max = to_int(E.max() * max_int)
#     print(f'static {c_int_type} iE_max = {iE_max};')
#     X /= max_int
#     Y /= max_int
# elif args.type == 'normal':
